...
class Section1:
    ...
    def partD(
        ...
        answer["cv"] = cv
        answer["scores"] = results_dict
        answer["explain_kfold_vs_shuffle_split"] = 'Pro : Offers flexibility in the choice of the train/test split size, allowing for more diverse splits Con: Less computationally efficient than k-fold cross-validation, and the randomness in splits might introduce variability in evaluation metrics'
        return answer
    ...
    def partE(
        ...
        for k in k_values:
            ...
            for key, values in scores.items():
                if key == 'test_score':
                    results_dict['mean_accuracy'] = values.mean()
                    results_dict['std_accuracy'] = values.std()
                    
            answer[k] = {'scores': results_dict, 'cv': cv, 'clf': clf}
    ...
    def partF(
        ...
        Part_D_var = part_D['scores']['std_accuracy'] ** 2
    ...
    def partG(
        ...
        # Fit the initial classifier on the entire training dataset
        clf.fit(X, y)
        ...
        "accuracy_orig_full_training": accuracies["accuracy_orig_full_training"],
        "accuracy_best_full_training": accuracies["accuracy_best_full_training"],
        "accuracy_orig_full_testing": accuracies["accuracy_orig_full_testing"],
        "accuracy_best_full_testing": accuracies["accuracy_best_full_testing"]
        }
